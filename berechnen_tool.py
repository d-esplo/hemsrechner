...
# Titel
st.title('Berechnungstool ohne HEMS')
st.header('Stromkosten einsparen mit einer PV-Anlage, BS, EV und WP')
...
if "Wärmepumpe" in selection:
    ...
    if 'Batteriespeicher' in selection:
        ...
        df_plt = df_evbs.copy()
         ...
         df_b = berechnen_ev.mit_pv(df, pv, homeoffice)
         ergebnisse = berechnen_ev.ersparnis_pv(df_b, anlage_groesse, strompreis)
         plot_data = ['PV Ertrag', 'Strombedarf', 'überschuss', 'netzbezug', 'EV Ladung']
         df_plt = df_b.copy()

    berechnen_ev.print_ersparnis_st(ergebnisse)
    ...
    plot_data = ['PV Ertrag', 'Strombedarf', 'überschuss', 'netzbezug', 'battery_soc']
    df_plt = df_pv.copy()
...
# 1 Tag - pro Stunde
''
st.subheader(":green[Plots]", divider=True)
